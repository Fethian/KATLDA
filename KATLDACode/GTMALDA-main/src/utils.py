...
def test_features_choose(rel_adj_mat, features_embedding):
    ...
    device = rel_adj_mat.device

    # Perform NMF and move the result to the same device
    R, D = nmf(A, lncSimi, disSimi, 0.01, 128, 500)
    RD = torch.tensor(np.vstack((R, D))).to(device)

    # Construct HNet and move the result to the same device
    x = constructHNet(A, lncSimi, disSimi).to(device)

    # Initialize and move the model to the same device
    model = GTN(7).to(device)
    b = model(x).squeeze().to(device)

    # Concatenate features and move to the same device
    features_embedding = [features_embedding.to(device), RD]
