...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        scores = torch.matmul(Q, K.transpose(-1, -2)) / torch.sqrt(torch.tensor(self.depth,
                                                                                dtype=torch.float32))  # 所以scores的shape最终是(1024, 128, 128),它对Q和K的乘法做了规范化处理,使得点积在0-1范围内。
...
class GraphTransformerLayer(nn.Module):
    ...
    def __init__(self, in_dim, hidden_dim, fout_dim, num_heads, dropout, layer_norm=False, batch_norm=True,
                 residual=True,
...
class GTM_net(nn.Module):
    def __init__(self, args, X):
        ...
        self.layers = nn.ModuleList([GraphTransformerLayer(in_dim, out_dim, fout_dim, head_num, dropout,
                                                           self.layer_norm, self.batch_norm, self.residual, args) for _
                                     in range(self.Sa - 1)])  # 256,128,64,8,0.4,true,false,true,9
        self.layers.append(
