...
def PredictScore(train_matrix, x, edge_index, args):
    train_matrix = train_matrix.contiguous().to(device)
    x = x.to(device)
...
def cross_validation_experiment(A, lncSimi, disSimi, args):
    ...
    for k in range(args.k_fold):
        ...
        pre_matrix[tuple(np.array(index[k]).T)] = predict_y_proba[
            tuple(np.array(index[k]).T)]  # 从预测分数矩阵中取出验证集的预测结果 只返回相应的预测分数
        A = np.array(A)
...
def main(args):
    ...
    # -------------------------
    result, score, drug_len, dis_len, tprs, fprs, aucs, precisions, recalls, auprs = cross_validation_experiment(A,
                                                                                                                 lncSimi,
                                                                                                                 disSimi,
                                                                                                                 args)
